...
import { 
  ...
  Waves, Zap, Thermometer, Droplets, Stethoscope
} from 'lucide-react';
  ...
  return (
    <div className={`w-full h-full flex flex-col overflow-hidden text-slate-800`}>
      ...
      <div className="flex-shrink-0 p-4 pb-2">
          <div className={`rounded-2xl p-5 flex items-center justify-between shadow-sm border relative overflow-hidden bg-white border-gray-200`}>
              ...
              <div className="flex-1 z-10 flex flex-col h-full justify-between pr-4">
                  <h2 className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-3 flex items-center gap-1.5">
                      <PieChart size={12} /> 状态概览
                  </h2>
                  
                  <div className="grid grid-cols-2 gap-2">
                      {chartData.filter(d => d.count > 0).map(d => (
                          <div key={d.key} className={`flex items-center justify-between px-2 py-1.5 rounded-lg border bg-slate-50 border-gray-100`}>
                              ...
                              <span className={`text-xs font-black text-slate-900 ml-1`}>{d.count}</span>
                          </div>
                      ))}
                  </div>
              </div>
              ...
              <div className="relative w-28 h-28 flex-shrink-0 z-10 mr-2">
                  <svg width="100%" height="100%" viewBox="0 0 100 100" className="transform -rotate-90 relative">
                      <circle cx="50" cy="50" r={radius} fill="transparent" stroke={'#f1f5f9'} strokeWidth="10" strokeLinecap="round" />
                      {segments.map((seg, i) => (
                          seg.count > 0 && <circle key={i} cx="50" cy="50" r={radius} fill="transparent" stroke={seg.color} strokeWidth="10" strokeDasharray={seg.dashArray} strokeDashoffset={seg.dashOffset} strokeLinecap="round" />
                      ))}
                  </svg>
                  <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                      ...
                      <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mt-0.5">台设备</span>
                  </div>
              </div>
          </div>
      </div>
      ...
      <div className="px-4 py-2 flex items-center gap-3 flex-shrink-0">
          {viewMode === 'devices' && (
              <button 
                ...
                className={`p-2.5 rounded-xl border transition-all bg-white border-gray-200 text-slate-600 hover:bg-gray-50 hover:text-slate-900 shadow-sm`}
                title="返回项目列表"
              >
                  <ArrowLeft size={18} />
              </button>
          ...
          <div className={`flex-1 flex items-center px-4 py-2.5 rounded-xl border bg-white border-gray-200 shadow-sm focus-within:ring-2 focus-within:ring-blue-500/20 transition-all`}>
              ...
              <input 
                  ...
                  className={`bg-transparent outline-none text-sm w-full font-medium placeholder-slate-400 text-slate-900`}
              />
          </div>
      ...
      {/* List Header */}
      <div className="px-5 py-2 text-xs font-bold text-slate-500 uppercase flex justify-between items-center tracking-wide">
          <span>{viewMode === 'projects' ? '项目列表' : selectedProjectName}</span>
          <span className="bg-slate-100 px-2 py-0.5 rounded text-[10px]">{viewMode === 'projects' ? projectStats.length : deviceList.length} 项</span>
      </div>
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar px-4 pb-20 space-y-3">
          {viewMode === 'projects' ? (
              ...
              projectStats.map(project => {
                  ...
                  return (
                      <div 
                          ...
                          className="p-5 rounded-2xl border bg-white border-gray-100 shadow-sm hover:shadow-md hover:border-blue-500/30 transition-all cursor-pointer group"
                      >
                          <div className="flex justify-between items-start mb-4">
                              <div className="flex-1 min-w-0 pr-4">
                                  <div className="font-bold text-base text-slate-900 truncate group-hover:text-blue-600 transition-colors">{project.name}</div>
                                  <div className="text-xs text-slate-400 mt-1 truncate">{project.description}</div>
                              </div>
                              <div className="flex items-center gap-1.5 bg-slate-50 text-slate-600 px-2.5 py-1 rounded-lg border border-slate-100">
                                  <Layers size={14} className="text-slate-400" />
                                  <span className="text-xs font-bold">{total} <span className="text-[10px] font-normal text-slate-400">台</span></span>
                              </div>
                          </div>
                          ...
                          {total > 0 ? (
                            <>
                              <div className="h-2 w-full rounded-full flex overflow-hidden mb-3 bg-slate-100">
                              ...
                              <div className="flex items-center gap-3 text-xs overflow-x-auto no-scrollbar pb-1">
                                   {project.counts[AlarmLevel.CRITICAL] > 0 && (
                                       <div className="flex items-center gap-1.5 font-bold text-red-700 bg-red-50 px-2 py-1 rounded-md whitespace-nowrap border border-red-100">
                                   ...
                                   {project.counts[AlarmLevel.DANGER] > 0 && (
                                       <div className="flex items-center gap-1.5 font-bold text-orange-700 bg-orange-50 px-2 py-1 rounded-md whitespace-nowrap border border-orange-100">
                                   ...
                                   {project.counts[AlarmLevel.WARNING] > 0 && (
                                       <div className="flex items-center gap-1.5 font-bold text-yellow-700 bg-yellow-50 px-2 py-1 rounded-md whitespace-nowrap border border-yellow-100">
                                   ...
                                   {project.counts[AlarmLevel.NORMAL] > 0 && (
                                       <div className="flex items-center gap-1.5 font-bold text-green-700 bg-green-50 px-2 py-1 rounded-md whitespace-nowrap border border-green-100">
                                   ...
                                   {project.counts[AlarmLevel.NO_DATA] > 0 && (
                                       <div className="flex items-center gap-1.5 font-bold text-slate-600 bg-slate-50 px-2 py-1 rounded-md whitespace-nowrap border border-slate-100">
                              ...
                              <div className="flex items-center gap-2 text-xs text-slate-400 bg-slate-50 p-2 rounded-lg justify-center border border-dashed border-slate-200">
              ...
              // Device List - Enhanced with Signal Values
              deviceList.map(device => {
                  ...
                  return (
                      <div 
                          ...
                          className={`p-4 rounded-xl border flex flex-col gap-4 transition-all bg-white border-gray-100 shadow-sm hover:shadow-md hover:border-blue-500/30 cursor-pointer group`}
                      >
                            ...
                            <div className="flex items-center gap-3 flex-1 min-w-0">
                                <div className={`w-10 h-10 rounded-lg overflow-hidden flex-shrink-0 bg-gray-100 border border-gray-200`}>
                                    {device.customImage ? (
                                        ...
                                        <div className="w-full h-full flex items-center justify-center"><LayoutGrid size={18} className="text-slate-300" /></div>
                                    )}
                                </div>
                                <div className="min-w-0">
                                    <div className={`font-bold text-sm truncate text-slate-900 group-hover:text-blue-600 transition-colors`}>{device.name}</div>
                                    <div className="text-xs text-slate-400 mt-0.5 truncate">{device.station}</div>
                                </div>
                            </div>
                            
                            {/* Prominent Status Badge */}
                            <div className={`flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-bold border ${statusConfig.className}`}>
                          ...
                          <div className="grid grid-cols-2 gap-2 bg-slate-50 rounded-xl p-3 border border-slate-100">
                                <div className="flex flex-col p-1">
                                    <span className="text-[10px] text-slate-400 font-bold mb-0.5">UHF 局放</span>
                                ...
                                <div className="flex flex-col p-1">
                                    <span className="text-[10px] text-slate-400 font-bold mb-0.5">TEV 局放</span>
                                    <div className="flex items-baseline gap-1">
                                ...
                                <div className="flex flex-col p-1">
                                    <span className="text-[10px] text-slate-400 font-bold mb-0.5">环境温度</span>
                                    <div className="flex items-baseline gap-1">
                                ...
                                <div className="flex flex-col p-1">
                                    <span className="text-[10px] text-slate-400 font-bold mb-0.5">环境湿度</span>
                                    <div className="flex items-baseline gap-1">
                          ...
                          <button 
                            ...
                            className="w-full py-2.5 rounded-xl border border-indigo-100 bg-indigo-50 text-indigo-600 font-bold text-xs flex items-center justify-center gap-2 hover:bg-indigo-100 hover:border-indigo-200 transition-colors active:scale-[0.98]"
                          >
                              <Stethoscope size={14} />
                              智能诊断
                          </button>
