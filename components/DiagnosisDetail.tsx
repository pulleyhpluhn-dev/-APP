...
import { 
  ArrowLeft, Activity, Zap, Waves, Maximize2, Rotate3d, Loader2, AudioWaveform, Thermometer, Droplets, Sparkles
} from 'lucide-react';
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar p-4">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 pb-4">
              ...
              <div className={cardClass}>
                  ...
                  <div className="flex-1 min-h-[300px] p-2 relative flex flex-col">
                      ...
                      <div className="flex justify-center pb-2">
                          <button 
                              onClick={() => {
                                  const payload = {
                                      projectName: '西湖区高压变电站监测',
                                      deviceId: sensorSn || sensorName,
                                      timestamp: Date.now(),
                                      chartType: 'PRPD 二维相位图谱(幅值-相位-频次)解析'
                                  };
                                  window.dispatchEvent(new CustomEvent('OPEN_AI_CHART_ANALYSIS', { detail: payload }));
                              }}
                              className={`flex items-center gap-1 px-3 py-1.5 rounded-full text-xs font-bold transition-all shadow-sm
                                  ${isDark ? 'bg-indigo-500/20 text-indigo-400 hover:bg-indigo-500/30 border border-indigo-500/30' : 'bg-indigo-50 text-indigo-600 hover:bg-indigo-100 border border-indigo-100'}
                              `}
                              title="AI 图谱诊断"
                          >
                              <Sparkles size={12} />
                          ...
                          <button 
                              onClick={() => {
                                  const payload = {
                                      ...
                                      deviceId: sensorSn || sensorName,
                                      timestamp: Date.now(),
                                      chartType: 'PRPS 三维图谱(周期-相位-幅值)连续性分析'
                                  };
                                  window.dispatchEvent(new CustomEvent('OPEN_AI_CHART_ANALYSIS', { detail: payload }));
                              }}
                              className={`flex items-center gap-1 px-3 py-1.5 rounded-full text-xs font-bold transition-all shadow-sm
